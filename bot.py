...
def create_app():
    ...
    app.config['IS_READY'] = False
    app.config['EXCHANGE'] = None
    ...
    try:
        ...
        time.sleep(3)
        app.config['IS_READY'] = True
        app.config['EXCHANGE'] = exchange
    ...
    def get_current_position(symbol):
        try:
            positions = app.config['EXCHANGE'].fetch_positions([symbol])
            for p in positions:
    ...
    def close_position(symbol, current_side):
        try:
            ...
            amount = app.config['EXCHANGE'].fetch_position(symbol)['contracts']
            if amount == 0:
                return
            logger.info(f"🚪 ポジション決済: {current_side.upper()} {amount}")
            app.config['EXCHANGE'].create_order(
                symbol=symbol,
                type='market',
                side=side,
                amount=abs(amount),
    ...
    def place_order(symbol, side):
        ...
        try:
            ...
            if side.lower() == 'buy':
                tp_price_raw = price * 1.001
                sl_price_raw = price * 0.997
            else:
                tp_price_raw = price * 0.999
                sl_price_raw = price * 1.003
